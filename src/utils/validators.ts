...
export function validateTitle(title: string): string | null {
  ...
  if (trimmed.length < 4) return 'El título debe tener al menos 4 caracteres.';
  ...
  if (!hasVowel) return 'El título debe contener palabras válidas con vocales.';
  ...
  if (words.length < 2) return 'El título debe tener al menos dos palabras.';
...
export function validateDescription(description: string): string | null {
  ...
  // Debe tener vocales
  const hasVowel = /[aeiouáéíóúü]/i.test(trimmed);
  if (!hasVowel) return 'La descripción debe contener palabras con vocales.';
...
export function validateLocation(location_id: string): string | null {
  if (!location_id) return 'La ubicación es obligatoria.';
  return null;
}
...
export function validateComments(comments: string): string | null {
  ...
  // Debe tener vocales
  const hasVowel = /[aeiouáéíóúü]/i.test(trimmed);
  if (!hasVowel) return 'Los comentarios deben contener palabras con vocales.';
