...
export interface ComplianceByLocation {
  location_id: string;
  compliance: number;
}
...
export async function getCountByStatus(
  ...
  for (const r of rows) {
    ...
    const normalized =
      s === 'Pendiente' || s === 'En Ejecución' || s === 'Finalizadas'
        ? (s as TicketStatus)
        : null;
...
// Cuenta tickets agrupando por location_id | assignee | requester
export async function getCountByField(
  ...
  const result: CountByFieldDTO[] = Array.from(counter.entries()).map(
    ([key, count]) => ({
      key,
      count,
    })
...
export async function getDashboardKpis(
  ...
  const { data, error } = await applyFilters<TicketLite>(
    buildBaseSelect(
      `status, created_at, closed_at, is_preventive, priority, due_date, planned, repair_time_h`
    ),
    filters
  );
  if (error) throw new Error(error.message);
  ...
  const openCount = rows.filter(
    (r) => r.status === 'Pendiente' || r.status === 'En Ejecución'
  ).length;

  // Vencidas
  const nowIso = new Date().toISOString();
  const overdueCount = rows.filter(
    (r) => !!r.due_date && r.due_date < nowIso && r.status !== 'Finalizadas'
  ).length;
  ...
  const prevClosed = rows.filter(
    (r) => r.is_preventive && r.status === 'Finalizadas'
  ).length;
  const prevPlanned =
    rows.filter((r) => r.is_preventive && (r.planned ?? true)).length || 1;
  const preventiveCompliance = Math.round((prevClosed / prevPlanned) * 100);

  // Backlog crítico
  const criticalBacklog = rows.filter(
    (r) => r.priority === 'Crítica' && r.status !== 'Finalizadas'
  ).length;
...
export async function getMttrTrend(
  filters?: ReportFilters
): Promise<MttrTrendPoint[]> {
  const { data, error } = await applyFilters<TicketLite>(
...
export async function getPreventiveVsCorrective(
  filters?: ReportFilters
): Promise<MonthSplit[]> {
  const { data, error } = await applyFilters<TicketLite>(
  ...
  for (const r of rows) {
    ...
    const monthsDiff =
      (nowD.getFullYear() - dt.getFullYear()) * 12 +
      (nowD.getMonth() - dt.getMonth());
...
export async function getPreventiveComplianceByLocation(
  filters?: ReportFilters
): Promise<ComplianceByLocation[]> {
  const { data, error } = await applyFilters<TicketLite>(
    buildBaseSelect(`location_id, is_preventive, status, planned`),
    filters
  );
  if (error) throw new Error(error.message);
  const rows = data ?? [];
  ...
  for (const r of rows) {
    ...
    const key = String(r.location_id ?? '(Sin ubicación)');
  ...
  return Array.from(map.entries())
    .map(([location_id, { closed, planned }]) => ({
      location_id,
...
export async function getTechnicianLoad(
  ...
  for (const r of rows) {
    ...
    if (r.status !== 'Finalizadas')
      map.set(key, (map.get(key) ?? 0) + Math.max(0, h));
  }
  ...
  toStackedBars(months: MonthSplit[]): BarChartData {
    return {
      ...
      datasets: [
        {
          label: 'Preventivo',
          data: months.map((m) => m.preventive),
          backgroundColor: '#10b981',
        },
        {
          label: 'Correctivo',
          data: months.map((m) => m.corrective),
