  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        className="group flex h-10 items-center gap-2 rounded-full border border-slate-200 bg-white px-1.5 pr-2 shadow-sm transition hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:border-slate-600 dark:bg-slate-900 dark:hover:bg-slate-800 dark:focus-visible:ring-indigo-400"
      >
        <span className="grid h-7 w-7 place-items-center rounded-full bg-slate-900 text-[11px] font-semibold text-white dark:bg-slate-800">
          {initials}
        </span>
        <span className="hidden min-w-0 sm:block">
          <span className="block max-w-[150px] truncate text-left text-sm font-semibold text-slate-900 dark:text-slate-100">
            {fullName}
          </span>
          <span className="block max-w-[150px] truncate text-left text-[11px] text-slate-500 dark:text-slate-400">
            {primaryRole}
          </span>
        </span>
        <ChevronDown
          className={`h-4 w-4 text-slate-500 transition dark:text-slate-400 ${open ? 'rotate-180' : ''}`}
        />
      </button>
