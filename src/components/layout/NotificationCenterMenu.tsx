...
export default function NotificationCenterMenu() {
  ...
  const desktopPanel = (
    <motion.div
      ...
      className="absolute right-0 z-[70] mt-2 hidden w-[24rem] max-w-[calc(100vw-1rem)] origin-top-right rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900 md:block"
    >
      {panelBody}
    </motion.div>
  );
  ...
  return (
    <div className="relative">
      ...
      <AnimatePresence>
        {open ? desktopBackdrop : null}
      </AnimatePresence>
      <AnimatePresence>
        {open ? desktopPanel : null}
      </AnimatePresence>
