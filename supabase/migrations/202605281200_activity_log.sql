-- Activity log (bitácora de auditoría) — rollout incremental para entornos desplegados.
-- Baseline canónico: sql/modules/core_cmms/17_activity_log.sql
-- Crea tabla append-only, captura por triggers, RPCs de lectura/registro, RLS solo
-- lectura por permiso, retención opcional, y siembra el módulo RBAC 'logs'.
-- Idempotente: seguro de re-ejecutar.

-- -----------------------------------------------------------------------------
-- 0) Permisos del módulo 'logs' (sembrar + asignar a Administrator)
-- -----------------------------------------------------------------------------

INSERT INTO public.permissions (id, resource, action, code, label, description, is_active, created_at)
VALUES
  (gen_random_uuid(), 'logs', 'read'::permission_action, 'logs:read', 'Ver bitácora de actividad', 'Acceso de solo lectura a la bitácora de auditoría', TRUE, NOW()),
  (gen_random_uuid(), 'logs', 'export'::permission_action, 'logs:export', 'Exportar bitácora de actividad', 'Permite exportar la bitácora de auditoría', TRUE, NOW())
...
WHERE r.name = 'Administrator'
  AND p.code IN ('logs:read', 'logs:export')
...
BEGIN
  IF v_actor IS NOT NULL THEN
    SELECT
      COALESCE(NULLIF(trim(concat_ws(' ', u.name, u.last_name)), ''), u.email),
      r.name
...
-- -----------------------------------------------------------------------------
-- 5) RPCs: registro de app + lectura con filtros
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 8) RPCs RBAC con logging semántico (junction/catálogo no cubierto por triggers)
-- -----------------------------------------------------------------------------
