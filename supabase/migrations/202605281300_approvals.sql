-- Approval processes + ticket close validation — rollout incremental.
-- Baseline canónico: sql/modules/core_cmms/18_approvals.sql
-- Crea procesos de aprobación (aprobadores/solicitantes), solicitudes de
-- validación, RPCs de flujo, guard de transición a 'Finalizadas', RLS, y siembra
-- el módulo RBAC 'approvals'. Idempotente.

-- -----------------------------------------------------------------------------
-- 0) Permisos del módulo 'approvals'
-- -----------------------------------------------------------------------------

INSERT INTO public.permissions (id, resource, action, code, label, description, is_active, created_at)
VALUES
  (gen_random_uuid(), 'approvals', 'read'::permission_action, 'approvals:read', 'Ver procesos de aprobación', 'Acceso de lectura al módulo de aprobaciones', TRUE, NOW()),
  (gen_random_uuid(), 'approvals', 'create'::permission_action, 'approvals:create', 'Crear procesos de aprobación', NULL, TRUE, NOW()),
  (gen_random_uuid(), 'approvals', 'full_access'::permission_action, 'approvals:full_access', 'Acceso total a procesos de aprobación', 'Crear/editar procesos y administrar aprobadores y solicitantes', TRUE, NOW())
...
WHERE r.name = 'Administrator'
  AND p.code IN ('approvals:read', 'approvals:create', 'approvals:full_access')
